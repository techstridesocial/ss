...
async function addMockPaymentData() {
  const pool = new Pool({
  ...
  try {
    console.log('🔄 Adding mock payment data for dashboard influencers...');
    ...
    // Add payment data for each influencer
    for (const [displayName, paymentData] of Object.entries(mockPaymentDetails)) {
      console.log(`\n💳 Adding payment data for ${displayName}...`);
      ...
      const existingPayment = await pool.query(`
        SELECT id FROM influencer_payments WHERE influencer_id = $1
      `, [influencerId]);
      
      if (existingPayment.rows.length > 0) {
        console.log(`✅ Payment data already exists for ${displayName}`);
        continue;
      }
      ...
      console.log(`✅ Added ${paymentData.method} payment method for ${displayName}`);
    }
    ...
    console.log('\n🎉 Mock payment data added successfully!');
    
  } catch (error) {
    console.error('❌ Error adding mock payment data:', error);
    process.exit(1);
  } finally {
    await pool.end();
  }

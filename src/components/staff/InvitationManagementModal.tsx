...
export default function UserManagementModal({ isOpen, onClose }: UserManagementModalProps) {
  ...
  const mockInvitations: Invitation[] = [
    ...
    },
    {
      ...
      expiresAt: new Date(Date.now() + 6 * 24 * 60 * 60 * 1000).toISOString(),
      invitedBy: 'STAFF',
      invitedAt: new Date(Date.now() - 4 * 60 * 60 * 1000).toISOString()
    }
  ]

  // Mock users data (users who have accepted invitations)
  const mockUsers = [
    {
      id: '1',
      email: 'sarah.wilson@example.com',
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div className="flex space-x-1">
                    {[
                      ...
                      { value: 'revoked', label: 'Cancelled', count: filteredInvitations.filter(i => i.status === 'revoked').length }
                    ].map((filterOption) => (
                      <button
